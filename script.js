  let students = new Map([
    ...
    ["Mohamed Fahmi", "Task 07", "Option 1"],
    ["Mohamed Fahmi", "Task 01", "Option 2"],
    ["Alaa Ahmed", "Task 01", "Option 2"],
    ["Abdelrahman Shemies", "Task 06", "Option 1"],
    ["Nathalie Nader", "Task 01", "Option 1"],
    ["Mariam Ahmed", "Task 02", "Option 1"],
      ]);
 
    let uniqStudents= Array.from(new Set(students) );
     let displayNames = "";
    for (let i = 0; i < uniqStudents.length; i++) {
        displayNames += 
        `<tr>
              <td>`+ i + `</td>
              <td>`+ uniqStudents[i][0] + `</td>
              <td>`+ uniqStudents [i][1] + `</td>
        </tr>`
        }
    document.getElementById("uneqNames").innerHTML = displayNames;
    console.table(uniqStudents);
